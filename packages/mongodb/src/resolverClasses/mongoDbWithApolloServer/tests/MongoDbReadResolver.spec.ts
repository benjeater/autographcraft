...
import { MongoDbReadResolver } from '../MongoDbReadResolver';
import type { HookInFunction } from '../../types';
import {
  DEFAULT_VALUES,
  READ_HOOK_POINTS_IN_ORDER,
  getAllHookMocks,
  getHookInFile,
  getStandardUser,
  getTestSetup,
  type ReadTestSetup,
} from './MongoDbReadResolver.data';

describe('MongoDbReadResolver', () => {
  let setup: ReadTestSetup;
  ...
  it('should be defined', () => {
    expect(MongoDbReadResolver).toBeDefined();
  });

  describe('happy path', () => {
    it('should read a document without any hooks or auth failures', async () => {
      // Arrange
      const { initialisationParams, findByIdMock, toObjectMock } = setup;

      // Act
      const resolver = new MongoDbReadResolver(initialisationParams);
      const result = await resolver.read();

      // Assert
      expect(findByIdMock).toHaveBeenCalledTimes(1);
      expect(findByIdMock).toHaveBeenCalledWith(
        DEFAULT_VALUES.TEST_DOCUMENT_ID
      );
    ...
    it('should authorise the caller architecturally and per document', async () => {
      ...
      ).toHaveBeenCalledWith(initialisationParams.context, databaseDocument);
    });
    ...
    it('should remove fields that the caller is not permitted to read', async () => {
      ...
      expect(result).toEqual({
        ...
        firstName: getStandardUser().firstName,
      });
    });
  });
  ...
  describe('hook in points', () => {
    it('should run every hook point in the documented order and not the error hook', async () => {
      ...
      // Assert
      const callOrders = READ_HOOK_POINTS_IN_ORDER.map((hookPoint) => {
      ...
      expect(hooks[HookInNames.PRE_COMMIT]).toHaveBeenCalledTimes(0);
      expect(hooks[HookInNames.POST_COMMIT]).toHaveBeenCalledTimes(0);
    ...
    it('should pass the fetched document to hooks that run after the fetch', async () => {
      // Arrange
      const { initialisationParams, databaseDocument } = setup;
      const { hookInFiles, hooks } = getAllHookMocks();
      initialisationParams.hookInFiles = hookInFiles;
    ...
    it('should stop running hooks at the point the resolver throws', async () => {
      ...
      // Act
      const resolver = new MongoDbReadResolver(initialisationParams);
      const action = async () => await resolver.read();
    ...
    it('should propagate an error thrown by a hook', async () => {
      ...
      expect(findByIdMock).toHaveBeenCalledTimes(0);
    });
  });
  ...
  describe('validation and authorisation failures', () => {
    ...
    it('should throw a NotFoundError when the document does not exist', async () => {
      ...
      // Assert
      await expect(action).rejects.toThrow(
        `Document with id ${DEFAULT_VALUES.TEST_DOCUMENT_ID} does not exist, or you do not have permission to access it`
      );
      await expect(action).rejects.toMatchObject({
    ...
    it('should throw a NotAuthorisedError when the caller cannot access the document', async () => {
      // Arrange
      const { initialisationParams, toObjectMock } = setup;
      ...
      expect(toObjectMock).toHaveBeenCalledTimes(0);
    });
  });
  ...
  describe('error handling', () => {
    ...
    it('should log the message of an Error that is thrown', async () => {
      // Arrange
      const { initialisationParams, findByIdMock, logger } = setup;
      findByIdMock.mockRejectedValue(new Error('Database unavailable'));
    ...
    it('should rethrow a GraphQLError without rewrapping it', async () => {
      ...
      findByIdMock.mockRejectedValue(originalError);
    ...
    it('should handle a thrown value that is not an Error', async () => {
      // Arrange
      const { initialisationParams, findByIdMock, logger } = setup;
      findByIdMock.mockImplementation(() => Promise.reject('a string failure'));
